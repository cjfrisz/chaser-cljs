...
(ns chaser-cljs.core
  ...
  (:require [dommy.core :refer (append! listen! remove!)]
            [dommy.attrs :refer (set-attr!)]
            [chaser-cljs.coords :refer (make-coords
                                        coords-get-x coords-get-y
                                        coords-update-x 
                                        coords-update-y)]
            [chaser-cljs.exit :refer (make-exit
                                      exit-get-x exit-get-y
                                      make-exit-renderer)]
...
(defn init-canvas!
  ...
  [game-env]
  (append! (sel1 game-div-id) (node [:canvas {:id game-canvas-id}]))
...
(defn init-game-env []
  (let [game-map (build-map map-size)
        player (make-player (player-start-coords game-map))]
    (make-game-env 
      game-map
      player
      (make-exit (exit-start-coords game-map player))
      (make-game-map-renderer)
      (make-player-renderer)
      (make-exit-renderer)
      (make-key-stream))))
          ...
          (if (nil? (seq key*))
              (do
                ...
                (render (game-env-get-player-renderer game-env)
                        ctx
                        game-env)
                (render (game-env-get-exit-renderer game-env)
                        ctx
                        game-env)
                (swap! global-game-env
